...
export default {
    ...
    methods: {
        async getAccountByEmail() {
            try {
                this.detailAccount = await authService.getAccountByEmail(this.$route.query.email);
            } catch (error) {
                alert("Có lỗi xảy ra khi lấy thông tin tài khoản người dùng");
            }
        },
    ...
    <div class="container-fluid" style="background-color: rgb(237, 234, 225);padding: 20px;">
        ...
        <div class="row" style="background-color: white; margin-top: 20px;">
            <div class="mt-3 text-center">
                ...
                <input type="email" id="input_email" v-model="input_email" @keyup.enter="searchAccount()">
                <i class="fa-solid fa-magnifying-glass" id="icon_search" @click="searchAccount()"></i>
            </div>
            <h4 class="text-center mt-3">Có {{ detailAccount == "" ? 0 : 1 }} kết quả tìm kiếm</h4>
            <table style="width: 70%; margin: 0 auto;" class="table table-success table-striped table-bordered mt-3">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{ detailAccount.phonenumber }}</td>
                        <td>{{ detailAccount.createdAt }}</td>
                        <td><i class="fa-solid fa-trash" @click="deleteUserAccount(detailAccount.email)"></i></td>
                    </tr>
                </tbody>
            </table>
        </div>
